...
class TestAdvancedIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Initialiser la base de données dans un fichier temporaire
        ...
        # Créer les calculateurs / moteur sur cette base
    ...
    def tearDown(self):
        ...
        # Fermer proprement la connexion SQLite utilisée par les tests
        try:
            if getattr(self.db_manager, "connection", None):
                self.db_manager.disconnect()
        except Exception as e:
            print(f"⚠️ Impossible de fermer la connexion DB en tearDown: {e}")
        ...
        try:
            ...
            # Sous Windows, un handle peut parfois rester ouvert; on logue mais on n'échoue pas le test
            print(f"⚠️ Impossible de supprimer {self.db_path} (fichier encore utilisé).")

        # Essayer de supprimer le dossier temporaire (optionnel)
        try:
            if os.path.isdir(self.temp_dir):
                os.rmdir(self.temp_dir)
        except OSError:
            # Dossier non vide ou déjà supprimé → pas grave pour les tests
            pass
    ...
    def test_advanced_cost_factors_integration(self):
        ...
        self.assertEqual(training_rate, 35.0)  # Valeur attendue pour casual_dining
    ...
    def test_weighted_formulas_accuracy(self):
        ...
        # On garde les résumés pour comparer les thèmes entre eux
        summaries_by_theme = {}
        ...
        for theme in themes_to_test:
            test_input = RestaurantInputs(
                ...
                kitchen_workstations=8,  # champs ajoutés
                staff_experience_level='standard'  # champs ajoutés
            )

            # Calculer avec formules pondérées
            summary = self.calculator.calculate_all_costs(test_input)
            summaries_by_theme[theme] = summary
        ...
        # Fine dining devrait avoir des coûts plus élevés que fast food
        fast = summaries_by_theme['fast_food']
        fine = summaries_by_theme['fine_dining']

        self.assertGreater(fine.staff_costs, fast.staff_costs)
        self.assertGreater(fine.operational_costs, fast.operational_costs)
    ...
    def test_scenario_comparison_integration(self):
        ...
        scenarios = {
            'Fast Food': RestaurantInputs(
                ...
                kitchen_workstations=6,              # update
                staff_experience_level='standard'    # update
            ),
    ...
    def test_batch_processing_performance(self):
        ...
        for i in range(5):
            inputs = RestaurantInputs(
                ...
                kitchen_workstations = 6,  # new field
                staff_experience_level = 'standard'  # new field
            )
    ...
    def test_error_handling_robustness(self):
        ...
        invalid_inputs = RestaurantInputs(
            ...
            location_rent_sqm=0,
            kitchen_workstations=1,  # new invalide field
            staff_experience_level='unknown'  # new field
        )
